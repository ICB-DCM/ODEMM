% generates model and estimates parameters for the RRE model with one
% parameter for all variances
...
M.u{1,e} = [1]; % to differentiate subpopulations
M.u{2,e} = [0];     
% distribution assumption
for s = 1:2
    M.distribution{s,e} = 'logn_median'; % 'norm', 'logn_median'
end
% initialize weights
M.sym.w{1,e} = ones(size(D(e).t(:)))*xi(5);
M.sym.w{2,e} = ones(size(D(e).t(:)))*(1-xi(5));

% intialize scaling and offset parameters
M.sym.scaling{r,e} = sym('1'); % no additional scaling parameter
M.sym.offset{r,e} = sym('0'); % no additional offset parameter
%%
[conditions,D] = collectConditions(D,M);
options.sigmas = 'only-one';
...
save('./project/results/results_RRE_onlyone','M','D','parameters','conditions','options')