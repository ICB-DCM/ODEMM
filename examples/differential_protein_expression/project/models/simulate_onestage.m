% simulate_onestage.m is the matlab interface to the cvodes mex
...
% USAGE:
% ======
% [...] = simulate_onestage(tout,theta)
% [...] = simulate_onestage(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_onestage(...)
%
% INPUTS:
% =======
...
function varargout = simulate_onestage(varargin)
...
if(nargin>=4)
    if(isempty(varargin{4}));
        data=amidata(length(tout),2,0,options_ami.nmaxevent,length(kappa));
    else
...
sol = ami_onestage(tout,theta(1:5),kappa(1:0),options_ami,plist,pbar,xscale,data);
if(options_ami.sensi==1)
