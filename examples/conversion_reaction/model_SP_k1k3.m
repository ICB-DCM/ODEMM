...
parameters.name = {'log_{10}mlog(k_{1,1})','mlog(k_{1,2})','log(k_2)',...
    'log_{10}m(log(k_3))','log_{10}(\sigma_{k_1}))','log(\sigma_{k_3}))','log_{10}(\sigma_{noise}))',......
    'w_1'};

parameters.number = length(parameters.name);
parameters.min = [-3*ones(4,1);-3;-3;-3;0];
parameters.max = [ 3*ones(4,1);2;2;2;1];

%%
M.name = 'CR_SP_k1k3';
M.n_subpop = 2; %number of subpopulations
M.model = @(T,theta,u) simulate_SP_CR_k1k3(T,theta,[]);
...
% initialize weights
M.sym.w{1,e} = ones(size(D(e).t(:)))*xi(8);
M.sym.w{2,e} = ones(size(D(e).t(:)))*(1-xi(8));

M.sym.scaling{r,e} = sym('1');
M.sym.offset{r,e} = sym('0');
M.sym.sigma_noise{r,e} = [10.^(xi(7))];
...
save('./project/results/results_SP_k1k3','M','D','parameters','conditions','options')