...
load('./project/results/geneExp_MA_Zechner','M','D','parameters','conditions')
xi = parameters.MS.par(:,1);
options_plot.model.col{1} = [245,211,40]./255;
...
D_.y = [];
plotODEMix(D_,M,xi,[],options_plot,[],fh);
ah=axes('position',[.15,.13,0.7813,0],'visible','off');
line([0,1],[0,1],'parent',ah,'linewidth',0.6,'Color','k');
ah=axes('position',[.15,.13,0,0.85],'visible','off');
line([0,1],[0,1],'parent',ah,'linewidth',0.6,'Color','k');
...
legend('off')
set(gcf, 'PaperUnits','centimeters', 'PaperPosition',[0 0 9 3])
...
%%
xi_true = log10([10,10,20,1,5,0.1]);
figure('name','Profiles Zechner')
names = {'k_1','k_{2}','k_3','k_4','k_5'};
for s = [1:5]
    subplot(1,5,s);
    plot(10.^parameters.P(s).par(s,:),parameters.P(s).R,'Color',[245,211,40]./255,'LineWidth',1.3); hold on;
    if s == 2
        plot([10.^xi_true(2),10.^xi_true(2)],[0,1],'Color',color.true,'LineWidth',1.3); hold on;
        plot([10.^xi_true(3),10.^xi_true(3)],[0,1],'Color',color.true,'LineWidth',1.3); hold on;
    elseif s > 2
        plot([10.^xi_true(s+1),10.^xi_true(s+1)],[0,1],'Color',color.true,'LineWidth',1.3); hold on;
    elseif s == 1
        plot([10.^xi_true(1),10.^xi_true(1)],[0,1],'Color',color.true,'LineWidth',1.3); hold on;
    end
    ylim([0,1.01])
    %xlim(xlimits{s});
    ylabel('likelihood ratio','fontsize',fs)
...
xlabel(names{s},'fontsize',fs)
set(gcf, 'PaperUnits','centimeters', 'PaperPosition',[0 0 15 3])
feval('print', '-dpdf',['./project/figures/profiles_Zechner']);
