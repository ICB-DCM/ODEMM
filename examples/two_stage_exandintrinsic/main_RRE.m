...
save ./results/geneExp_RRE parameters D M options conditions 
...
options.MS.parameter_index = 1:7;
...
parameters.MS.BIC = -2*parameters.MS.logPost+ ...
    log(sum(sum(~isnan(D(1).y))))*parameters.number;
save ./results/geneExp_RRE parameters D M options conditions 

options.MS.parameter_index = [4,6];
options.MS.MAP_index = 2;
parameters_2 = getParameterProfiles(parameters,@(xi) ...
    logLikelihood(xi,M,D,options,conditions),options_PL);
save ./results/geneExp_RRE_2ndmode parameters_2 D M options conditions 
