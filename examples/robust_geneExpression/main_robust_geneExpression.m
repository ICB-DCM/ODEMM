...
for iDist = 4%1:length(distributions)
    ...
    M.model = @(T,theta,u) simulate_geneExpression_MA(T,theta,[]);
    ...
    if strcmp(distributions{iDist},'skew_norm')
        M.sym.delta{1,e} = 10.^xi(8)-1e3;
        M.sym.delta{2,e} = 10.^xi(8)-1e3;
    end
    M.u{1,e} = [1]; % subpop 1
    M.u{2,e} = [0]; % subpop 2
    % initialize weights
    ...
    M.name = [M.distribution{1,1} '_geneExpression'];
    ...
    plotODEMM(D,M,xi)
    save(['./results/results_geneExpression_SSA_' distributions{iDist}])
    
%     for jDist = 4%1:length(distributions)
%         load(['data/data_geneExpression_MA_' distributions{jDist}])
%         [conditions,D] = collectConditions(D,M);
%         parameters = getMultiStarts(parameters,@(xi) ...
%             logLikelihood_extend(xi,M,D,options,conditions),options.MS);
%         save(['./results/results_geneExpression_MA_' ...
