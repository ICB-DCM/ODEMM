...
for m = [1,3,4]
    ...
    if noiseFlag
        print('-depsc',['./figures/nooutlier_boxplot2_' modelnames{m}]);
    else
        print('-depsc',['./figures/nooutlier_noise_boxplot2_' modelnames{m}]);
    end
    if m == 1
        fh = robustSimStudy_boxplots3(BICs,MSEs,converged,t_cpus,false);
    else
        fh = robustSimStudy_boxplots3(BICs,MSEs,converged,t_cpus,false);
    end
    figure(fh)
    fh.PaperPosition=[0 0 4 7];
    if noiseFlag
    ...
    fh.PaperPosition=[0 0 4 9];
    if noiseFlag
        print('-depsc',['./figures/outlier_noise_boxplot2_' modelnames{m}]);
    else
        print('-depsc',['./figures/outlier_boxplot2_' modelnames{m}]);
    end
